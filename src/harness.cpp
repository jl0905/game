    ...
    void DumpState() const {
        ...
        if (gs.screen == Screen::Battle) {
            ...
            std::printf("battle: heroPos=(%.2f,%.2f,%.2f) yaw=%.3f pitch=%.3f "
                        ...
                        "arrows=%d wall=%d climbs=%d rain=%d night=%d kills=%d horses=%d order=%s form=%s anchordist=%.1f over=%d won=%d foe=%s banners=%d/%d reserves=%d/%d duel=%d shots=%d quiver=%d\n",
...
int RunScript(const char* path) {
    ...
    while (std::getline(*in, line)) {
        ...
        if (cmd == "quit") break;
            ...
            // Challenge the lord to single combat (V102) — D at deployment.
            BattleInput bin; bin.duel = true;
            h.Step(CampaignInput{}, bin);
        } else if (cmd == "warcry") {
